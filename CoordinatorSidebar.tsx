...
const CoordinatorSidebar: React.FC<CoordinatorSidebarProps> = ({ userName, className }) => {
  const location = useLocation();
  const { logout } = useAuth();
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false);
  
  const navItems = [
    { icon: FileText, label: 'Research Publications', path: '/coordinator/publications' },
    { icon: List, label: 'Exam Results', path: '/coordinator/exam-results' },
    { icon: BookOpen, label: 'SWAYAM Courses', path: '/coordinator/swayam-courses' },
    { icon: Calendar, label: 'Exam Dates', path: '/coordinator/exam-dates' },
  ];
    ...
    <>
      ...
      <aside className={cn("bg-white border-r border-gray-100 w-[220px] flex flex-col transition-all", className)}>
        <div className="p-6 border-b border-gray-100 flex flex-col items-center">
          <h2 className="text-xl font-semibold text-blue-600">Coordinator Portal</h2>
